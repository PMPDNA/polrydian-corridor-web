...
-- Fix prevent_role_self_elevation function
DROP FUNCTION IF EXISTS public.prevent_role_self_elevation(uuid, app_role, app_role);
...
-- Fix assign_initial_admin function
...
DROP FUNCTION IF EXISTS public.sync_linkedin_article_to_articles(uuid);
...
BEGIN
  ...
  -- For now, return base64 encoded (will be enhanced with proper AES-GCM)
  -- This is a placeholder for full AES-GCM implementation
...
BEGIN
  ...
  -- For now, return as text (will be enhanced with proper AES-GCM decryption)
...
-- Add trigger for role changes if it doesn't exist
