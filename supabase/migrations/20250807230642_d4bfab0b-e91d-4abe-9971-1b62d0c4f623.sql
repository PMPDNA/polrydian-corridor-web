-- Phase A: Database improvements (fixed)
...
-- Step 3: Add missing updated_at triggers (check for existence first)
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_consultation_bookings_updated_at') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_articles_slug_trigger') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'prevent_articles_corruption') THEN
