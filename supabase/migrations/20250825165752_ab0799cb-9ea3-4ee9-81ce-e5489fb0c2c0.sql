-- Create storage policies for secure file handling (fixed)
...
DROP POLICY IF EXISTS "Authenticated users can upload to mobile-uploads" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can view their own mobile uploads" ON storage.objects;
...
DROP POLICY IF EXISTS "Admins can manage all mobile uploads" ON storage.objects;
