...
-- Add unique constraint to prevent duplicate image paths
ALTER TABLE public.images 
ADD CONSTRAINT IF NOT EXISTS unique_file_path UNIQUE (file_path);

-- Enhance RLS policies for better security
DROP POLICY IF EXISTS "Enhanced image access control" ON public.images;
CREATE POLICY "Enhanced image access control"
ON public.images
FOR ALL
USING (
  CASE 
    ...
    WHEN auth.uid() = uploaded_by THEN true
    ELSE false
  END
)
WITH CHECK (
