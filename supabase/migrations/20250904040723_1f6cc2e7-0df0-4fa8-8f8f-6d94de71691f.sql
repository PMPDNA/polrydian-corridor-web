...
-- Enhanced visitor analytics table (fixing column reference)
DROP TABLE IF EXISTS public.new_visitor_analytics;
CREATE TABLE public.new_visitor_analytics (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  created_at timestamptz DEFAULT now(),
  path text NOT NULL,
  referrer text,
...
-- Migrate existing data if visitor_analytics exists
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'visitor_analytics') THEN
    INSERT INTO public.new_visitor_analytics (
      id, created_at, path, referrer, utm_source, utm_medium, utm_campaign,
      device_type, browser, country, region, ip_hash, session_id, user_id
    )
    SELECT 
    ...
    DROP TABLE public.visitor_analytics;
  END IF;
END $$;

ALTER TABLE public.new_visitor_analytics RENAME TO visitor_analytics;
